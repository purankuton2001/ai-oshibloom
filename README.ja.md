...
AI OshiBloom は、同じ要件定義から 2 つのモデルが作った 2 つのアプリの片方です。もう 1 つは [castconjure](https://github.com/purankuton2001/castconjure)（クリップ型、Claude Fable 5.1 製）。どちらももう片方のおまけではありません。比較は Board で。
...
現在の23件のテストは、キュー、中止処理、費用確保、再生開始通知、プロバイダーへの入力、HTTP/WebSocket、連続ライブの障害復旧、キャラ・音声の更新を検証します。生成品質は実際の動画で別途確認しています。
...
同梱ライブラリの条件は[第三者ライセンス表示](public/THIRD_PARTY_NOTICES.txt)に記載しています。
